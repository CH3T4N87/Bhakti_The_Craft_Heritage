...
app.use(express.static(path.join(__dirname, '../kachra')));
    newItem.save()
    .then(item => res.redirect('/rt1.html'))
...
app.post('/signin', (req, res) => {
    ...
    Item.findOne({ Username: Username, Password: Password })
        .then(user => {
            if (user) {
                // If user exists, redirect to rt.html
                res.redirect('/rt.html');
            } else {
...
app.get('/rt.html', (req, res) => {
    res.sendFile(path.join(__dirname, 'rt.html'));
});
